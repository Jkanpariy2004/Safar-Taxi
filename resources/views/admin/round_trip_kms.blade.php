...
<div class="card card-outline card-info rounded-0">
	...
	<div class="card-body">
		<form action="round_log" method="post" id="cab-form">
            @csrf
            <div class="form-group">		
               ...
               <div class="form-group col-12 col-lg-4">
                ...
                <select class="form-select custom-select2" name="from[]" aria-label="Default select example">
               ...
               <div class="form-group col-12 col-lg-4">
                <label for="destiny" class="control-label">Destination</label> 
                <select class="form-select custom-select2" name="destiny[]" aria-label="Default select example">
                ...
                <div class="form-group col-6 col-lg-4">
                    <label for="kms" class="control-label">Total KMs</label>
                    <input name="kms[]" id="kms" type="number" class="form-control rounded-0" value="" required>
                </div><br>
         
                <div class="container"><a id="remove" class="btn removeButton btn-outline-danger">Remove</a>
            </div><br></div>
            </div>
            ...
            <a id="add" style="display:none" class="btn btn-outline-warning">Adds another</a>
...
<script>
    ...
    checkbox.addEventListener("change", function() {
        ...
        if (this.checked) {
            ...
            let div='<div class="row city_div mb-5" style="border:1px solid black;background:grey" id="city_div"><div class="form-group col-12 col-lg-4"><label for="from" class="control-label">From</label> <select class="form-select custom-select2" name="from[]" aria-label="Default select example"><option value="">Select Pick UP City</option>@foreach($cities as $city)<option value="{{$city->city}}">{{$city->city}}</option>@endforeach</select></div><div class="form-group col-12 col-lg-4"><label for="destiny" class="control-label">Destination</label> <select class="form-select custom-select2" name="destiny[]" aria-label="Default select example"><option value="">Select Drop City</option>@foreach($cities as $city)<option value="{{$city->city}}">{{$city->city}}</option>@endforeach</select></div><div class="form-group col-6 col-lg-4"><label for="kms" class="control-label">Total KMs</label><input name="kms[]" id="kms" type="number" class="form-control rounded-0" value="" required></div><br><div class="container"><a id="remove" class="btn removeButton btn-outline-danger">Remove</a></div><br></div>'
